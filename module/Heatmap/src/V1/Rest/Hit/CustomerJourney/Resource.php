...
namespace Heatmap\V1\Rest\Hit\CustomerJourney;
...
class Resource extends DbConnectedResource
{
    ...
    {
        return $this->createCustomerJourneyCollection($data);
    }

    private function createCustomerJourneyCollection($data = [])
    {
        $customerId = $this->getEvent()->getRouteParam('customer_id');

        $select = $this->table->getSelectCustomerJourney($customerId);
